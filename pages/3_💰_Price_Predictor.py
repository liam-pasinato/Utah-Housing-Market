...
with st.form(key='MLform'):
    st.write('##### Specify values and press "Get Price Prediction" to recieve a price estimation for a house with specified features:')
    col1, col2, col3 = st.columns(3, gap='medium')

    #Input boxes in columns
    with col1:
        sq_ft = st.number_input("Square Footage", min_value=300, max_value=33000, value=6000, step=500)
        num_car = st.number_input("Car Garage", min_value=0, max_value=30, value=1, step=1)
        st.write('###### Predictions from *DataRobot*')

    with col2:
        num_bdrm = st.number_input("Bedrooms", min_value=1, max_value=12, value=3, step=1)
        num_patio = st.number_input("Patios", min_value=0, max_value=5, value=1, step=1)
    
    with col3:
        num_bath = st.number_input("Bathrooms", min_value=1, max_value=18, value=2, step=1)
...
xAxis = st.selectbox("Select X-Axis:", ('sq_ft', 'price', 'acres', 'bathrooms', 'bedrooms'), index=0)

fig1 = st.container().empty()

#Scatter plot
fig = px.scatter(predictions, x=xAxis, y='price_PREDICTION', 
    labels={
...
if submit_button: 
    ...
    with predict_expl.container():
        st.write('#### Showing the 3 features with the largest impact on your prediction:')
